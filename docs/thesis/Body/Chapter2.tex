...
\begin{enumerate}
    ...
    \item Self-consistency methods fail on ``consistent hallucinations''
    \item Uncertainty methods alone are insufficient as models can confidently hallucinate
\end{enumerate}

LOGIC-HALT addresses these gaps by combining metamorphic testing (to expose inconsistencies through semantic variants), NLI-based consistency analysis (to detect contradictions), and information-theoretic metrics (to capture uncertainty and complexity). This hybrid approach provides a more robust, reference-free framework for hallucination detection in logical reasoning tasks.
